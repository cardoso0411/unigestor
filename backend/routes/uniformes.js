...
router.post("/deliveries", (req, res) => {
  ...
  db.beginTransaction((err) => {
    if (err) return res.status(500).json(err);
    const qDel = "DELETE FROM uniform_deliveries WHERE employee_id = ? AND item = ?";
    db.query(qDel, [employee_id, item], (err1) => {
      if (err1) {
      ...
      const qIns = `
        ...
        VALUES (?, ?, ?)
      `;
      db.query(qIns, [employee_id, item, observation], (err2) => {
        if (err2) {
